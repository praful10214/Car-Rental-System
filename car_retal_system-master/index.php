...
<br>
        <section class="menu-content">
            ...
            if (! isset($_SESSION['login_client'])){
            $sql = "SELECT * FROM cars WHERE car_availability=1 and registered=1 ORDER BY Reg_time DESC LIMIT 10";
            }
            else {
                 $lg =  $_SESSION['login_client'];
                $sql = "SELECT * FROM cars cr , clientcars ccr where ( (ccr.car_id = cr.car_id) and (ccr.client_username  ='$lg') )";
            }
            ...
            if($res->num_rows > 0) {
                while($row1 = mysqli_fetch_assoc($res)){
                    ...
                    ?>
            <a href="info.php?id=<?php echo($car_id) ?>">
            <div class="sub-menu">
            ...
            <?php
                if (isset($_SESSION['login_client'])) {
                    {
                    if ($car_av == 1){
                        echo "<a href='Admin/activate_car.php?id=$car_id&dbit=1' ><button class='delete_buttons'>Delete</button></a>";
                    }
                    else  {
                        echo "<a href='Admin/activate_car.php?id=$car_id&dbit=1'><button class='delete_buttons' disabled=''>Booked</button></a>";
                        }
                    }
                }    
...
<h1> No cars available  </h1>
            ...
            <div><?php
            if(! isset($_SESSION['login_client'])){
           echo " <h3 style='text-align:center;'>Rent your dream car</h3>";
            echo "<section class='menu-content'>";
            $sql0 = "SELECT Id,Category from car_categories";
            $res0 = $conn->query($sql0);
            if ($res0->num_rows ){
                echo "<select id='select_cat' class='form-select custom-select' style='padding: 10px; border-radius: 15px; color: #495057; border: 2px solid #ced4da; box-shadow: 0 4px 6px rgba(0,0,0,0.1);'>";
                echo "<option value='0' selected>No Categories</option>";

                while ($data = mysqli_fetch_assoc($res0)){
                    $val =  $data['Category'];
                    if ($val != NULL)
        ...
        <section class="menu-content" id='filter_items'>
